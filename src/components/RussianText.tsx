...
const nounStems: Array<{ stem: string; gender: Gender }> = [
  ...'учител,инженер,студент,рабоч,начальник,директор,телефон,звонок,сосед,ключ,этаж,офис,друг,голос,вопрос,ответ,язык,текст,диалог,фильм,хлеб,диван,телевизор,порт,стол,стул,муж,дом,пап,врач,номер,чай,парк,мёд,мяч,мир,мост,брат,дедушк,дяд,отец,человек,ребёнок,ребенок,родител,сын,мальчик'.split(',').map((stem) => ({ stem, gender: 'masculine' as const })),
  ...'професси,квартир,лестниц,комнат,зарплат,больниц,компани,грамматик,музык,стать,гитар,ошибк,дорог,работ,школ,книг,газет,правд,фраз,двер,мам,панд,семь,сестр,бабушк,тёт,тет,девушк,доч,жен,женщин'.split(',').map((stem) => ({ stem, gender: 'feminine' as const })),
  ...'упражнен,письм,радио,мюсл,окн,мор,слов,мыл,семейств'.split(',').map((stem) => ({ stem, gender: 'neuter' as const })),
